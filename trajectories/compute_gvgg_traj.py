...
def compute_list(cam_dir, rel_mat, pose_file, threshold = 0.01, max_len = 30):
    ...
    for i in range(rel_matrix.shape[0]):
        ...
        if len(neg_indices) == 0:
            ...
            neg_indices += random.choices(neg_indices, k=2*max_len - len(neg_indices))
        #### positives #####
        rgb_paths_per = np.array(rgb_paths)[pos_indices].tolist()
        poses_per = np.array(poses)[pos_indices].tolist()
        pos_rgb_paths_all.append(rgb_paths_per)
        pos_indices_all.append(pos_indices)
        pos_depth_file_all.append(depth_file)
        pos_pose_all.append(poses_per)
        #### negatives #####
        rgb_paths_per = np.array(rgb_paths)[neg_indices].tolist()
        poses_per = np.array(poses)[neg_indices].tolist()
        neg_rgb_paths_all.append(rgb_paths_per)
        neg_indices_all.append(neg_indices)
...
def create_dataset(dataset_size, scene_name, mode, data_root, Train_Test_list):
    ...
    for i in range(dataset_size):
        ...
        for floor in floor_folders:
            ...
            pos_indices, pos_rgb_paths, pos_depth_file, pos_poses, neg_indices, neg_rgb_paths, neg_depth_file, neg_poses = compute_list(cam_dir, rel_mat, pose)
