...
def compute_list(cam_dir, rel_mat, pose_file, positives, negatives, floor, threshold = 0.01, max_len = 30, resolution=(224,224)):
    ...
    os.makedirs("HM3D2", exist_ok=True) 
    ...
    for i in range(rel_matrix.shape[0]):
        ...
        for j in range(len(pos_indices)):   
            ...
            for pos_index in pos_indices_copy:
                ...
                if not os.path.exists(f"HM3D2/{scene_seed}/{floor}/{str(pos_index)}_{str(i)}.npy"):
                    ...
                    os.makedirs(f"HM3D2/{scene_seed}/{floor}", exist_ok=True) 
                    bool_arr = src_mask == 255 
                    np.save(f"HM3D2/{scene_seed}/{floor}/{str(pos_index)}_{str(i)}.npy", bool_arr) 
                pos_covariant_paths.append(f"HM3D2/{scene_seed}/{floor}/{str(pos_index)}_{str(i)}.npy")
            ...
            for neg_index in neg_indices_copy:
                ...
                assert os.path.exists(covariant_path), f"File does not exist: {covariant_path}"
                if not os.path.exists(f"HM3D2/{scene_seed}/{floor}/{str(neg_index)}_{str(i)}.npy"):
                    left_region, right_region = extract_regions_from_stitched_image(covariant_path)
                    assert(left_region is not None and right_region is not None)
                    ref_mask = extract_green_mask(left_region)
                    src_mask = extract_green_mask(right_region)
                    ...
                    os.makedirs(f"HM3D2/{scene_seed}/{floor}", exist_ok=True) 
                    bool_arr = src_mask == 255 
                    np.save(f"HM3D2/{scene_seed}/{floor}/{str(neg_index)}_{str(i)}.npy", bool_arr) 
                neg_covariant_paths.append(f"HM3D2/{scene_seed}/{floor}/{str(neg_index)}_{str(i)}.npy")
            ...
            # if not os.path.exists(f"HM3D2/{scene_seed}/{floor}/{str(i)}_{str(i)}.npy"):
            #     np.save(f"HM3D2/{scene_seed}/{floor}/{str(i)}_{str(i)}.npy", ref_masks) 
            ref_covariant_paths.append(f"HM3D2/{scene_seed}/{floor}/{str(i)}_{str(i)}.npy")
...
print("#########: ", len(sequences), train_len, test_len)
...
save_dataset(dataset_train, "HM3D2_train/dataset_train.json")

dataset_test = create_dataset(test_len, scene_name, "test", data_root, Test_list)
print("len(dataset_test)::::", len(dataset_test))
save_dataset(dataset_test, "HM3D2_test/dataset_test.json")
