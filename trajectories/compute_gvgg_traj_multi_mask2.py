...
def compute_list(cam_dir, rel_mat, pose_file, positives, negatives, floor, threshold = 0.01, max_len = 30, resolution=(224,224)):
    ...
    os.makedirs("covision", exist_ok=True) 
    ...
    for i in range(rel_matrix.shape[0]):
        ...
        for j in range(len(pos_indices)):   
            ...
            for pos_index in pos_indices_copy:
                scene_seed = rgb_paths_per[0].split("/")[5]
                ...
                os.makedirs(f"covision/{scene_seed}/{floor}", exist_ok=True) 
                if not os.path.exists(f"covision/{scene_seed}/{floor}/{str(pos_index)}_{str(i)}.npy"):
                    bool_arr = src_mask == 255 
                    np.save(f"covision/{scene_seed}/{floor}/{str(pos_index)}_{str(i)}.npy", bool_arr) 
                pos_covariant_paths.append(f"covision/{scene_seed}/{floor}/{str(pos_index)}_{str(i)}.npy")
            ...
            for neg_index in neg_indices_copy:
                scene = rgb_paths_per[0].split("/")[5].split("-")[0]
                ...
                ref_masks = np.logical_or(ref_masks, ref_mask)
                os.makedirs(f"covision/{scene_seed}/{floor}", exist_ok=True) 
                if not os.path.exists(f"covision/{scene_seed}/{floor}/{str(neg_index)}_{str(i)}.npy"):
                    bool_arr = src_mask == 255 
                    np.save(f"covision/{scene_seed}/{floor}/{str(neg_index)}_{str(i)}.npy", bool_arr) 
                neg_covariant_paths.append(f"covision/{scene_seed}/{floor}/{str(neg_index)}_{str(i)}.npy")
            ...
            # if not os.path.exists(f"covision/{scene_seed}/{floor}/{str(i)}_{str(i)}.npy"):
            #     np.save(f"covision/{scene_seed}/{floor}/{str(i)}_{str(i)}.npy", ref_masks) 
            ref_covariant_paths.append(f"covision/{scene_seed}/{floor}/{str(i)}_{str(i)}.npy")
