    ...
    for i in range(rel_matrix.shape[0]):
        ...
        for j in range(len(pos_indices)):   
            ...
            for pos_index in pos_indices_copy:
                ...
                pos_covariant_paths.append(covariant_path)
            ...
            for neg_index in neg_indices_copy:
                ...
                assert os.path.exists(covariant_path), f"File does not exist: {covariant_path}"
                neg_covariant_paths.append(covariant_path)
        ...
        for j in range(len(pos_indices)):   
            ...
            neg_pose_all.append(poses_per)
            neg_covariants.append(neg_covariant_paths)  
    return query_indice, ref_rgb_paths_all, pos_indices_all, pos_rgb_paths_all, pos_depth_file_all, pos_pose_all, pos_covariants, neg_indices_all, neg_rgb_paths_all, neg_depth_file_all, neg_pose_all, neg_covariants, total_positive_len
...
def create_dataset(dataset_size, scene_name, mode, data_root, Train_Test_list):
    ...
    for i in range(dataset_size):
        ...
        for floor in floor_folders:
            ...
            query_indice, rgb_paths, pos_indices, pos_rgb_paths, pos_depth_file, pos_poses, pos_covariants, neg_indices, neg_rgb_paths, neg_depth_file, neg_poses, neg_covariants, len_positives = compute_list(cam_dir, rel_mat, pose, positive_indices, negative_indices)
