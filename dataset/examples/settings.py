...
default_sim_settings = {
    ...
    "equirect_rgba_sensor": False, # Modified
    "equirect_depth_sensor": False, # Modifed
...
def make_cfg(settings):
    ...
    # Override default lighting to use PBR light from habitat sim.
    ...
    def create_camera_spec(**kw_args):
        ...
        camera_sensor_spec.position = np.array([0, settings["sensor_height"], 0])
        for k in kw_args:
            setattr(camera_sensor_spec, k, kw_args[k])
        return camera_sensor_spec

    ## Putting depth_sensor at the front to make the process easier.
