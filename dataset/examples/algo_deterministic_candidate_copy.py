from test_runner_copy import TestRunner
...
from utils import pivot_candidates, get_candidate_svd, in_map #get_candidate_GA
...
def intersect_score_simple(runner, original_map, explored_map, stopping_map, candidate, candidate_pose, saved_depth, saved_color, saved_pose,
    ...
    print("inside intersect score simple")
    max_overlap = -np.inf
    max_weight = -np.inf
    cam2base_h = 1.5
    ...
    print("after draw_sight_rays")
    # c1_ind = my_function_c1()
    pcd_c1, pose_c1 = runner.convert_obs_to_pcd_v2(obs_c1, c1_pose)
    # print(pcd_c1)
    lower_limit = ((1080//2) - 1)*1920
    upper_limit = lower_limit + 1920

    print("after convert_obs_to_pcd_v2")
    ...
    for ind_2, svd_pose in enumerate(saved_pose):  ###### Need optimization here
        ...
        c2 = get_candidate_svd(runner, p2[1], p2, p2[4], original_map, scene_name, iter_number, iter)
        ...
        pcd_c2, pose_c2 = runner.convert_obs_to_pcd_v2(obs_c2, p2_temp)
        ...
        #Added by Taarun
        # pcd_intersection_score, pcd_union_score, iou, intersection_indices_cand, intersection_indices_svd =  runner.calculate_iou_point_cloud_v3(pcd_c1, pcd_c2, pose_c1, pose_c2)
        iou =  runner.calculate_iou_point_cloud_v4(scene_name, str(iter_number), runner,len(saved_pose), ind_2, c1, c2)
        # pcd_intersection_score, pcd_union_score, iou =  runner.calculate_iou_point_cloud_v3(pcd_c1, pcd_c2)

        # print("Here now line 206")
        world_lines2, grid_lines2 = draw_sight_rays(runner, [c2], 0, pivot_c2[1], scene_name, iter_number, iter + 0.5)
        
        # print("Here now line 208")
        if plot_flag:
            length = 100
            fig, ax = plt.subplots()
            ax.set_title(scene_name + " " + str(pivot_c1[1]))
...
def pick_best_candidate(
    ...
    # @TODO: Need optimization here
    print("Inside pick best_candidates")
    for i, candidate in enumerate(candidates):
        ...
        print("after focal length and rotation")
        ...
        print("Before intersect score simple")
    ...
    depth_dir = os.path.join("temp", "More_vis", str(scene_name), str(iter_number), "depth")
...
def algorithm(runner: TestRunner, floor_maps: list, heights: list, scene: str, SCENE_THRESHOLD: float, resume_flag=False, floor_flag=0) -> None:
    ...
    for i, floor in enumerate(floor_maps[floor_flag:]):
        ...
        saved_depths = np.load(os.path.join("SpatialReasoningDataset_HM3DA", "temp", "More_vis", scene, str(i), "saved_obs", "saved_dep.npy"), allow_pickle=True)
        ...
        while (np.sum(stopping_map) / np.sum(init_unexplored) >= SCENE_THRESHOLD) and (len(obs_array_sample_index) > 0) and total_trials > 0 and step < saved_depths.shape[0]:
            ...
            saved_depths = np.load(os.path.join("SpatialReasoningDataset_HM3DA", "temp", "More_vis", scene, str(i), "saved_obs", "saved_dep.npy"), allow_pickle=True)
            saved_colors = np.load(os.path.join("SpatialReasoningDataset_HM3DA", "temp", "More_vis", scene, str(i), "saved_obs", "saved_color.npy"), allow_pickle=True)
            saved_poses = np.load(os.path.join("SpatialReasoningDataset_HM3DA", "temp", "More_vis", scene, str(i), "saved_obs", "saved_pose.npy"), allow_pickle=True)
            ...
            # Pick the best
            print("Here now, before inside pick best candidates")
...
def process_scene(scene, floor_heights,scene_dir,resume_flag, scene_flag=None, floor_flag=None,SCENE_THRESHOLD=0.20):
    ...
    if dataset_flag == "gibson":
        ...
        default_sim_settings["scene"] = f"glb_files/hm3dGlb/{scene_name}"
...
def main(start=0, end=None, resume_flag=False, scene_flag=None, floor_flag=None):
    ...
    if dataset_flag == "gibson":
        ...
        # available_scenes = ["Rancocas", "Reyno", "Ribera", "Roane", "Roeville", "Rosser", "Roxboro", "Sanctuary", "Sands", "Sasakwa", "Sawpit", "scioto", "Seward", "Shelbiana", "Silas", "Sisters", "Sodaville", "Soldier", "Spencerville", "Spotswood", "Springhill", "Stanleyville", "Stilwell", "Stokes", "Sumas", "Superior", "Swormville", "Woonsocket"]
        # available_scenes = ["Maryhill", "Mesic", "Micanopy", "Mobridge", "Monson", "Mosinee", "Mosquito", "Nemacolin", "Nicut", "Nimmons", "Nuevo", "Parole", "Pettigrew", "Placida", "Pleasant", "Quantico", "Rancocas", "Reyno", "Roane", "Roeville", "Rosser", "Sanctuary", "Sands", "Sasakwa", "Sawpit", "Scioto", "Seward", "Shelbiana", "Silas", "Sisters", "Sodaville", "Soldier", "Spencerville", "Spotswood", "Springhill", "Stanleyville", "Stilwell", "Stokes", "Sumas", "Superior", "Swormville", "Woonsocket"]
        
        # available_scenes = ["Adrian", "Albertville", "Anaheim", "Andover", "Angiola", "Annawan", "Applewold", "Arkansaw", "Avonia", "Azusa", "Ballou", "Beach", "Bolton", "Bowlus", "Brevort", "Cantwell", "Capistrano", "Colebrook",
        # "Convoy", "Cooperstown", "Crandon", "Delton", "Denmark", "Dryville", "Dunmor", "Eagerville", "Eastville", "Edgemere", "Elmira", "Eudora", "Goffs", "Greigsville", "Hainesburg", "Hambleton", "Haxtun", "Hillsdale", "Hometown", "Hominy", "Kerrtown",
        # "Maryhill", "Mesic", "Micanopy", "Mobridge", "Monson", "Mosinee", "Mosquito", "Nemacolin", "Nicut", "Nimmons", "Nuevo", "Oyens", "Pablo", "Parole", "Pettigrew", "Placida", "Pleasant", "Quantico", "Rancocas", "Reyno", "Ribera", "Roane", "Roeville",
        # "Rosser", "Roxboro", "Sanctuary", "Sands", "Sasakwa", "Sawpit", "Scioto", "Seward", "Shelbiana", "Silas", "Sisters", "Sodaville", "Soldier", "Spencerville", "Spotswood", "Springhill", "Stanleyville", "Stilwell", "Stokes", "Sumas", "Superior", "Swormville", "Woonsocket"]

        # available_scenes = ["Adrian"]
        ...
        available_scenes = {filename[14:-4] for filename in glob.glob("glb_files/hm3dGlb/*.glb")}
        available_scenes = sorted(list(available_scenes))
        available_scenes = ["1hovphK64XQ.basis"]
