...
class TestRunner:
    ...
    def get_covariant_image(self, scene_name, floor_num, id1, position1, rotation1, id2, position2, rotation2):
        os.makedirs(os.path.join("temp", "More_vis", scene_name, floor_num, "covariant_images"), exist_ok=True)
        ...
        state.sensor_states['color_sensor'].rotation = rotation2
        agent.set_use_mask(True)
        agent.set_binary_image(True)
        agent.set_state(state, infer_sensor_states=False)
        binary_observation = self._sim.get_sensor_observations()
        #########################
        agent = self._sim.initialize_agent(0)
        ...
        agent.set_binary_image(False)
        agent.set_state(state, infer_sensor_states=False)
        compObs2 = self._sim.get_sensor_observations()

        state = agent.get_state()
        state.sensor_states['depth_sensor'].position = position2
        state.sensor_states['depth_sensor'].rotation = rotation2
        state.sensor_states['color_sensor'].position = position1
        state.sensor_states['color_sensor'].rotation = rotation1
        agent.set_use_mask(True)
        agent.set_binary_image(False)
        agent.set_state(state, infer_sensor_states=False)
        observation2 = self._sim.get_sensor_observations()
        ...
        agent.set_binary_image(True)
        agent.set_state(state, infer_sensor_states=False)
        binary_observation2 = self._sim.get_sensor_observations()

        # Draw and save images
        img_path = os.path.join("temp", "More_vis", scene_name, floor_num, "covariant_images")
        img1 = Image.fromarray(observation['color_sensor'])
        img1.save(os.path.join(img_path, f"{scene_name}_depth_{id1}_color_{id2}.png")
        img2 = Image.fromarray(observation2['color_sensor'])
        img2.save(os.path.join(img_path, f"{scene_name}_depth_{id2}_color_{id1}.png")

        binary_img_path = os.path.join(img_path, 'binary_images')
        os.makedirs(binary_img_path, exist_ok=True)
        binary_img1 = Image.fromarray(binary_observation['color_sensor'])
        binary_img1.save(os.path.join(binary_img_path, f"binary_{scene_name}_depth_{id1}_color_{id2}.png")
        binary_img2 = Image.fromarray(binary_observation2['color_sensor'])
        binary_img2.save(os.path.join(binary_img_path, f"binary_{scene_name}_depth_{id2}_color_{id1}.png")
        # fig, ax = plt.subplots(1, 2)
        # ax[0].set_title(f"{scene_name}_depth_{id1}_color_{id2}")
        # ax[0].imshow(np.array(observation["color_sensor"]))
        # ax[0].imshow(np.array(compObs["color_sensor"]))
        # ax[1].set_title(f"{scene_name}_depth_{id2}_color_{id1}")
        # ax[1].imshow(np.array(observation2["color_sensor"]))
        print(f"img1: {scene_name}_depth_{id1}_color_{id2}, img2: {scene_name}_depth_{id2}_color_{id1}")
        # fig.savefig(os.path.join("temp", "More_vis", scene_name, floor_num, "covariant_images", f"{scene_name}_depth_{id1}_color_{id2}.png"))
        # plt.close()
        
        
        
        # if id1 == 2 and id2 == 1 and floor_num == "1":
        #     # diff_o1_c1 = np.sum(np.any(compObs2['color_sensor'] != observation["color_sensor"], axis=-1)) 
        #     # diff_o1_c1 = np.sum(np.abs(compObs2['color_sensor'] - observation["color_sensor"]) > 50)
        #     # diff_o1_c1 = np.sum(np.any(np.abs(compObs2["color_sensor"] - observation["color_sensor"]) > 20, axis=-1))
            
        #     # print(f"diff_o1_c1: {diff_o1_c1}")
        #     image_pil = Image.fromarray(np.array(compObs["color_sensor"]))
        #     image_pil.save(os.path.join("temp", "More_vis", str(scene_name), "1", "compObs.png"), "PNG")

        #     image_pil2 = Image.fromarray(np.array(observation2["color_sensor"]))
        #     image_pil2.save(os.path.join("temp", "More_vis", str(scene_name), "1", "observation2.png"), "PNG")
        ...
        # print("compobs2", compObs2["color_sensor"])
        # assert()

        return compObs2['color_sensor'], compObs['color_sensor'], observation["color_sensor"], observation2["color_sensor"]
    ...
    def calculate_iou_point_cloud_v4(self, scene, floor_num, runner, ind1, ind2, candidate1, candidate2, thresh_val=0.05, H=1080, W=1920):
        # print(f"iou tolerance: {thresh_val}")
        device = "cuda" if torch.cuda.is_available() else "cpu"

        (_, pivot1, rotation1, _) = candidate1
        (_, pivot2, rotation2, _) = candidate2

        pivot1_copy = pivot1.copy()
        pivot2_copy = pivot2.copy()
        pivot1_copy[1] += 1.5
        pivot2_copy[1] += 1.5

        o1, o2, c1, c2 = runner.get_covariant_image(scene, floor_num, ind1, pivot1_copy, rotation1, ind2, pivot2_copy, rotation2)

        diff_o1_c1 = runner.get_green_pixels_count(o1, c1)
        diff_o2_c2 = runner.get_green_pixels_count(o2, c2)
        # diff_o1_c1 = np.sum(np.any(o1 != c1, axis=-1))  # Count differing pixels
        # diff_o2_c2 = np.sum(np.any(o2 != c2, axis=-1))  # Count differing pixels
        print("o1 value: ", diff_o1_c1)
        print("o2 value: ", diff_o2_c2)

        total_pixel1, total_pixel2 = o1.shape[0]* o1.shape[1], o2.shape[0]* o2.shape[1]
        
        iou = diff_o1_c1 * diff_o2_c2 / (total_pixel1 * total_pixel2)
    ...
    def get_green_pixels_count(self, img1, img2):
        ...
        red_diff = np.abs(img1_RGB[:, :, 0] - img2_RGB[:, :, 0])
        blue_diff = np.abs(img1_RGB[:, :, 2] - img2_RGB[:, :, 2])
        green_diff = np.abs(img1_RGB[:, :, 1] - img2_RGB[:, :, 1])

        mask = (red_diff < red_thresh) & (blue_diff < blue_thresh) & (green_diff > green_thresh)
        pixel_locations = np.array(np.argwhere(mask))
        green_pixels = pixel_locations.shape[0]
