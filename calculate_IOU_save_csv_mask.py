...
from dust3r.inference import loss_of_one_batch_mask  # noqa
...
def path_to_idx(image_path):
    return int(os.path.basename(image_path).split(".")[0].split("_")[-1])

# Function: Resize the mask to match the RGB image size
def align_mask_to_image(mask, image_rgb):
    if mask is None or image_rgb is None:
        return None

    # Get the dimensions of the mask and image
...
# Function: Extract specified regions from the stitched image
...
def _downsample_to_target(mask, target_resolution=(224, 224)):
    # 0.3 - 0.4s
    mask_pil = PIL.Image.fromarray(mask)

    resized_mask_pil = mask_pil.resize(target_resolution, PIL.Image.NEAREST)  # 适用于 mask，保持类别一致
    # ~0.3s    
    return resized_mask_pil
...
def get_result(model, scene, level, save_dir = "pairwise_results", device='cuda', batch_size = 1, size=224, silent=False, n_frame=10):
    ...
    total_res = {}
    covariant_images_dir = os.path.expandvars(f"data/gvgg/temp/More_vis/{scene}/{level}/covariant_images/")

    for i in range(len(images)-1):
        for j in range(i+1, len(images)):
            criterion = eval("BCELoss()").to(device)
            idx1 = path_to_idx(image_paths[i])
            idx2 = path_to_idx(image_paths[j])
            covariant_path = os.path.join(covariant_images_dir, scene.split("-")[0]+"_depth_"+str(idx2)+"_color_"+str(idx1)+".png")
            ...
            mask1 = extract_green_mask(left_region)
            mask2 = extract_green_mask(right_region)
            aligned_mask1 = align_mask_to_image(mask1, imread_cv2(image_paths[i]))
            aligned_mask2 = align_mask_to_image(mask2, imread_cv2(image_paths[j]))
            aligned_mask1 = _downsample_to_target(aligned_mask1, (size,size))
            aligned_mask2 = _downsample_to_target(aligned_mask2, (size,size))
            ...
            if bool(pred_prob2.item()) == int(gt[(gt['image_1'] == gt_path(i, scene, level)) & (gt['image_2'] == gt_path(j, scene, level))]["label"]):
                pred_mask2 = pred_ret['p_mask2s'][0].squeeze() > 0.5
                pred_mask2 = pred_mask2.cpu().numpy()
                gt_mask2 = np.array(aligned_mask2) == 255
                plt.imsave("gt_binary_mask.png", gt_mask2, cmap="gray")
                plt.imsave("pred_binary_mask.png", pred_mask2, cmap="gray")
    ...
    with open(save_dir + "_" + model_base + f"/{scene}_{level}.json", "w") as file:
...
def get_label(scene, level, model_base):
    ...
    pair_wise_results = f"pairwise_results_{model_base}"
...
if __name__ == '__main__':
    ...
    model_name = "outputs/checkpoint-last.pth"
    model_base = os.path.basename(model_name).split(".")[0]
    ...
    if not os.path.exists(f"pairwise_results_{model_base}/"):
        os.makedirs(f"pairwise_results_{model_base}/")
    scene_levels = {}
    for scene in scenes:
    ...
    # #     print(scene_name, scene_levels[scene_name])
        for i in range(len(levels)):
            if not os.path.exists(os.path.join(f"pairwise_results_{model_base}", f"{scene_name}_{i}.json")):
    ...
    for scene in tqdm(scenes, desc="Processing scenes"):
        ...
        for i in range(len(levels)):
            coordinates, over_diff, gt_diff, IOU = get_label(scene_name, i, model_base)
    ...
    with open(f"pair_wise_IOU_{model_base}_results.json", "w") as file:
        json.dump(results, file, indent=2)
    
    print("IOU final: ", iou_avg)
