...
def get_result(model, scene, level, save_dir = "pairwise_results", device='cuda', batch_size = 1, size=224, silent=False, n_frame=10):
    scene_path = os.path.expandvars(f"data/hvgg/parta/temp/More_vis/{scene}/{level}/saved_obs/")
...
def get_label(scene, level, model_base):
    scene_path = f"data/hvgg/parta/temp/More_vis/{scene}/{level}/saved_obs/"
...
if __name__ == '__main__':
    ...
    more_vis = os.path.expandvars("data/hvgg/parta/temp/More_vis")
    ...
    ################################################################
    results = {}
    more_vis = "data/hvgg/parta/temp/More_vis/"
    scenes = list(glob.glob(more_vis+"/*"))
    scenes = [scene for scene in scenes if os.path.basename(scene) in Test_list]
