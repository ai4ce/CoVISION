...
#include <ostream>
#include <iostream>
#include "esp/gfx/RenderResourceManager.h"
...
CameraSensor::CameraSensor(scene::SceneNode& cameraNode,
  ...
  // Sanity check
  // std::cout << "Constructing CameraSensor" << std::endl;
  CORRADE_ASSERT(
      cameraSensorSpec_,
...
void CameraSensor::setCovariantMask(bool useMask) {
  RenderResourceManager::getInstance().setUseMask(useMask);
}

void CameraSensor::setBinaryImage(bool binaryImage) {
  RenderResourceManager::getInstance().setBinaryImage(binaryImage);
}
...
bool CameraSensor::drawObservation(sim::Simulator& sim) {
  ...
  // std::cout << "drawing observation in CameraSensor" << std::endl;
  ...
  if (cameraSensorSpec_->sensorType == SensorType::Depth) {
    // std::cout << "CPP: SensorType is depth and storing depth texture to manager" << std::endl;
    Magnum::GL::Texture2D& depthTexture = renderTarget().getDepthTexture();
    RenderResourceManager::getInstance().setDepthTexture(depthTexture);
    RenderResourceManager::getInstance().setCam1NearPlane(cameraSensorSpec_->near);
    RenderResourceManager::getInstance().setCam1FarPlane(cameraSensorSpec_->far);
  }
