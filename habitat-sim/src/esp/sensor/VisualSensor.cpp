...
void VisualSensor::readObservation(Observation& obs) {
  ...
  // std::cout << "reading observation in visualsensor" << std::endl;
  if (buffer_ == nullptr) {
    // TODO: check if our sensor was resized and resize our buffer if needed
    // std::cout << "Buffer is empty when reading" << std::endl;
...
bool VisualSensor::getObservation(sim::Simulator& sim, Observation& obs) {
  ...
  // std::cout << "Drawing OBS in visual Sensor" << std::endl;
  drawObservation(sim);
  // std::cout << "Reading OBS in visual Sensor" << std::endl;
  readObservation(obs);
