...
PbrDrawable::PbrDrawable(scene::SceneNode& node,
  ...
  // std::cout << "Constructing PbrDrawable" << std::endl;
...
void PbrDrawable::draw(const Mn::Matrix4& transformationMatrix,
  ...
  // std::cout << "Drawing with PbrDrawable" << std::endl;
  ...
  // Getting required resources from first rendering pass.
  Magnum::GL::Texture2D* depthTexture = RenderResourceManager::getInstance().getDepthTexture();
  const Magnum::Matrix4& cam1ViewMatrix = RenderResourceManager::getInstance().getCam1ViewMatrix();
  const Magnum::Matrix4& cam1ProjectionMatrix = RenderResourceManager::getInstance().getCam1ProjectionMatrix();
  float cam1NearPlane = RenderResourceManager::getInstance().getCam1NearPlane();
  float cam1FarPlane = RenderResourceManager::getInstance().getCam1FarPlane();
  const bool useMask = RenderResourceManager::getInstance().getUseMask();
  const bool binaryImage = RenderResourceManager::getInstance().getBinaryImage();
  // Storing cam1 resources to manager.
  // Clean render resources.
  if (!useMask) {
    RenderResourceManager::getInstance().setDepthTextureToNull();
    RenderResourceManager::getInstance().resetCameras();
  }

  if (useMask && !depthTexture) {
    // std::cout << "CPP: No depth texture here so first render pass and save first cam" << std::endl;
    RenderResourceManager::getInstance().setCam1ViewMatrix(camera.cameraMatrix());
    RenderResourceManager::getInstance().setCam1ProjectionMatrix(camera.projectionMatrix());
  }
  ...
  (*shader_)
      ...
      .setUseDepthTexture(useMask)
      .setGetBinaryImage(binaryImage)
      .setEmissiveColor(matCache.emissiveColor)
      .setCam1ViewMatrix(cam1ViewMatrix)
      .setCam1ProjectionMatrix(cam1ProjectionMatrix)
      .setCam1NearPlane(cam1NearPlane)
      .setCam1FarPlane(cam1FarPlane);

  if (useMask && depthTexture) {
    // std::cout << "Retrieved depthTexture and binding to the fragshader" << std::endl;
    shader_->bindDepthTexture(*depthTexture);
  }
