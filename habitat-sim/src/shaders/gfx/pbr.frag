...
// Passing cam1 depth and cam1&2 matrices
uniform highp sampler2D camera1DepthTexture;
uniform float camera1NearPlane;
uniform float camera1FarPlane;
uniform bool useDepthTexture;
uniform bool getBinaryImage;

in highp vec4 camera1ClipSpace;
...
float linearizeDepth(float depth, float near, float far) {
    return (2.0 * near * far) / (far + near - depth * (far - near) * 2.0);
}
...
// "none", "Diff (l,n)", "F (l,h)", "G (l,v,h)", "D (h)", "Specular"
  ...
  highp vec3 camera1NDC = camera1ClipSpace.xyz / camera1ClipSpace.w;

  // Convert NDC to texture coordinates (0 to 1) for sampling
  highp vec2 camera1TexCoord = camera1NDC.xy * 0.5 + 0.5;

  // Check if the coordinates are within the valid range
  if (camera1TexCoord.x < 0.0 || camera1TexCoord.x > 1.0 || camera1TexCoord.y < 0.0 || camera1TexCoord.y > 1.0) {
    // Outside of camera1's frustrum, mark as not visible
    if (getBinaryImage) {
      fragmentColor = vec4(0.0,0.0,0.0, fragmentColor.a);
    }
    return;
  }

  // Sample the depth from `camera1`'s depth texture
  float camera1Depth = texture(camera1DepthTexture, camera1TexCoord).r;
  //float linearCamera1Depth = linearizeDepth(camera1Depth, camera1NearPlane, camera1FarPlane);

  // Compare depths to determine if both cameras can see the fragment
  if (camera1Depth+0.00001 < camera1NDC.z*0.5+0.5) {
    if (getBinaryImage) {
      fragmentColor = vec4(0.0,0.0,0.0, fragmentColor.a);
    }
    // Don't add extra color so do nothing.
    return;
  } else {
    // Pixels match in both cameras' views
    if (useDepthTexture) {
      vec3 maskColor = mix(fragmentColor.rgb, vec3(0.0,1.0,0.0), 0.25);
      fragmentColor = vec4(maskColor, fragmentColor.a);
      if (getBinaryImage) {
        fragmentColor = vec4(1.0,1.0,1.0, fragmentColor.a);
      }
    }
  }
}  // main
