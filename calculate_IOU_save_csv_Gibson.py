...
def get_result(model, scene, level, save_dir = "pairwise_results", device='cuda', batch_size = 1, size=224, silent=False, n_frame=10):
    ...
    with open(save_dir + "_" + model_base + f"_Gibson/{scene}_{level}.json", "w") as file:
...
def get_label(scene, level, model_base):
    ...
    pair_wise_results = f"pairwise_results_{model_base}_Gibson"
...
if __name__ == '__main__':
    ...
    model_name = "outputs/checkpoint-last.pth"  # change path according to the trained checkpoint folder
    ...
    if not os.path.exists(f"pairwise_results_{model_base}_Gibson/"):
        os.makedirs(f"pairwise_results_{model_base}_Gibson/")
    scene_levels = {}
    ...
    # #     print(scene_name, scene_levels[scene_name])
        for i in range(len(levels)):
            if not os.path.exists(os.path.join(f"pairwise_results_{model_base}_Gibson", f"{scene_name}_{i}.json")):
    ...
    with open(f"pair_wise_IOU_{model_base}_Gibson_results.json", "w") as file:
