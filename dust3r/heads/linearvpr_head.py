...
class LinearFeat(nn.Module):
    ...
    def __init__(self, net, has_conf=False, head_no=False, skip = False):
        ...
        # Shrink H and W
        self.conv2 = nn.Conv2d(64, 128, kernel_size=3, stride=2, padding=1)  # H/2, W/2
        self.conv3 = nn.Conv2d(128, 256, kernel_size=3, stride=2, padding=1) # H/4, W/4
        self.conv4 = nn.Conv2d(256, 512, kernel_size=3, stride=2, padding=1) # H/8, W/8
        self.conv5 = nn.Conv2d(512, 1024, kernel_size=3, stride=2, padding=1) # H/16, W/16
        self.conv6 = nn.Conv2d(1024, 2048, kernel_size=3, stride=2, padding=1) # H/32, W/32
    ...
    def forward(self, decout, img_shape, head_no): 
        ...
        if type(decout) is tuple:
            decout, img = decout
        tokens = decout[-1] # choose the output of the last layer, we have only one layer
        B, S, D = tokens.shape

        # extract 3D points
        feat = self.proj(tokens)  # torch.Size([64, 196, 1024]) # 196 is 14*14 patches, each path is 16*16 pixels
        ...
        if self.skip:
            skip_input = torch.cat([img.to(feat.device), feat[:, :3]], dim=1)
        ...
        feat = F.relu(self.bn5(self.conv5(feat)))  # B, 1024, H/16, W/16
        feat = self.conv6(feat)  # B, 2048, H/32, W/32
