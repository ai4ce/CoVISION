...
def back_projection_vwt(depthmap, camera_intrinsics, camera_pose, pseudo_focal=None):
    # defining image space coordinates
    # X -> (-1,1) and Y -> (1,-1)
    #Defining depth coordinates from ground truth results and reshaping them
    depth_scaled = (depthmap / np.max(depthmap.flatten()) * 65535).astype(np.uint16)
    H, W = depthmap.shape

    xs, ys = np.meshgrid(np.linspace(-1, 1, H), np.linspace(1, -1, W))
    xs = xs.reshape(1, H, W).astype(np.float32)
    ys = ys.reshape(1, H, W).astype(np.float32)
    depthmap = depthmap.reshape(1, H, W).astype(np.float32)

    #xys is the image space coordinate combined with the depth coordinates
    xys = np.vstack((xs * depthmap, ys * depthmap, -depthmap, np.ones(depthmap.shape)))
    xys = xys.reshape(4, -1)

    # #Intrinsic matrix as defined by habitat sim conventions
    ...
    #normalized image coordinate space -> camera space
    xy_c0 = np.matmul(np.linalg.inv(K), xys)

    #calculating the extrinsic matrix
    # import pdb; pdb.set_trace()
    ext_mat = calculate_extrinsic_matrix(camera_pose)
    # ext_mat = calculate_extrinsic_matrix(pose)

    #Camera space -> World space (3D global coordinates)
    xy_newimg = np.matmul(ext_mat, xy_c0)
    xy_newimgs = np.matmul(K, xy_newimg)
    xy_newimgs = xy_newimgs.T

    # Flatten the world coordinates and stack them with the RGB values
    points = xy_newimgs[:, :3].reshape(H, W, 3)
    valid_mask = (depthmap > 0.0)
    return points.astype(np.float32), valid_mask.squeeze(0), ext_mat

def calculate_extrinsic_matrix(pose):
    # print(f"pose: {pose}")
    # raise ValueError("")
    translation = pose[:3]
    # print("initial", translation)
    translation[1], translation[2] = translation[2], translation[1]  # translation[0] -> flips the axis
    # print("Ty in idx 1", translation)
    # translation[0], translation[2] = translation[2], translation[0]

    # translation[1] = 1.5 + translation[1]
    # translation[2] = off_z + translation[2] #0
    # translation[0] = off_x + translation[0] #2
    rot_quat = R.from_euler("zyx", pose[3:], degrees=True).as_quat()
...
def depthmap_to_absolute_camera_coordinates(depthmap, camera_intrinsics, camera_pose, **kw): # camera_pose: c2w
    ...
    Returns:
        pointmap of absolute coordinates (HxWx3 array), and a mask specifying valid pixels."""
    X_world, valid_mask, extrinsic = back_projection_vwt(depthmap, camera_intrinsics, camera_pose)

    # R_cam2world = np.float32(camera_params["R_cam2world"])
    # t_cam2world = np.float32(camera_params["t_cam2world"]).squeeze()
    # R_cam2world = camera_pose_m[:3, :3]
    # t_cam2world = camera_pose_m[:3, 3]

    # # Express in absolute coordinates (invalid depth values)
    # X_world = np.einsum("ik, vuk -> vui", R_cam2world, X_cam) + t_cam2world[None, None, :]
    return X_world, valid_mask, extrinsic
