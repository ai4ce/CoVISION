...
class HM3DDataset(BaseStereoViewDataset):
    ...
    def _get_views(self, idx, resolution, rng, from_tar = False, ref_view_id = None):
        ...
        assert os.path.exists(pos_depth_file[1:]), f"File {pos_depth_file[1:]} does not exist"
        assert os.path.exists(neg_depth_file[1:]), f"File {neg_depth_file[1:]} does not exist"
        ...
        try:
            pos_depth_list = np.load(pos_depth_file[1:])
            pos_depth_list = pos_depth_list[np.array(pos_indices)]
            neg_depth_list = np.load(neg_depth_file[1:])
            neg_depth_list = neg_depth_list[np.array(neg_indices)]
        except Exception as e:
            print(f"Error loading depth data: {e}")
            print(f"pos_depth_file: {pos_depth_file}")
            print(f"neg_depth_file: {neg_depth_file}")
            raise
