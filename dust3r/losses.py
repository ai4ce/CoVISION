...
class TripletLoss(MultiLoss):
    def __init__(self, margin=0.2):
        super().__init__()
        self.margin = margin

    def get_name(self):
        return f'TripletLoss(margin={self.margin})'

    def compute_loss(self, gt1, gt2s, pred1, pred2s):
        """
        Compute triplet loss.

        Args:
            gt1: Ground truth for query (not used in triplet loss)
            gt2s: Ground truth for positives and negatives (not used in triplet loss)
            pred1: Query embeddings, shape (batch_size, feature_dim)
            pred2s: List of embeddings for positives and negatives.
                    It contains 23 tensors (first 7 are positives, last 16 are negatives).
        ...
        pair_labels = torch.cat([gt["pair_label"].unsqueeze(1) for gt in gt2s], dim=1).to(pred1.device)  # (batch_size, 23)
        pred2s = torch.cat([p.unsqueeze(1) for p in pred2s], dim=1)  # (batch_size, N, feature_dim)

        # (3) pair_labels change to int32(1 represent positives, 0 represent negatives)
        pair_labels = pair_labels.to(dtype=torch.int32) 
        _, sorted_indices = torch.sort(pair_labels, descending=True, dim=1)  # make sure positives in the fronts
        sorted_pred2s = torch.gather(pred2s, dim=1, index=sorted_indices.unsqueeze(-1).expand(-1, -1, feature_dim))
        sorted_labels = torch.gather(pair_labels, dim=1, index=sorted_indices)  # sorted labels

        # (4)  positives and negatives
        pos_count = sorted_labels.sum(dim=1).long()  # count how many positives
        negatives_start_idx = pos_count.unsqueeze(1)  # negative starts from here

        positives = sorted_pred2s[:, :pos_count.max(), :]  # Positives: (batch_size, max_positives, feature_dim)
        negatives = sorted_pred2s[:, pos_count.max():, :]  # nagatives: (batch_size, max_negatives, feature_dim)

        assert(positives.shape[1]>0 and negatives.shape[1]>0)
        # Compute L2 distances
        query_expanded = pred1.unsqueeze(1)  # (batch_size, 1, feature_dim)
        pos_distances = torch.norm(query_expanded - positives, dim=2)  # (batch_size, 7)
        neg_distances = torch.norm(query_expanded - negatives, dim=2)  # (batch_size, 16)

        # Hardest positive (max distance)
        hardest_pos = torch.max(pos_distances, dim=1)[0]  # (batch_size,)

        # Hardest negative (min distance)
        hardest_neg = torch.min(neg_distances, dim=1)[0]  # (batch_size,)

        # Compute triplet loss
        loss = F.relu(hardest_pos - hardest_neg + self.margin).mean()

        # Return loss and details
        details = {self.get_name(): float(loss.detach().cpu().item())}
        return loss, details

class BCELoss(MultiLoss):
    def __init__(self):
        super().__init__()
