...
--train_dataset " \
4300 @ MVDataset(split='all', ROOT='trajectories/covision_train', aug_crop=16, mask_bg='rand', resolution=224, transform=ColorJitter, num_views=7, num_render_views=2, random_order = True, random_render_order = False, dps_name = 'dataset_train.json', random_nv_nr = [[7,2]])" \
--test_dataset " \
MVDataset(n_all=1000, split='all', ROOT='trajectories/covision_test', resolution=224, seed=777, fix_order=True, num_views=7, num_render_views=2, render_start = 8, dps_name = 'dataset_test.json', tb_name = 'mp3d_tdf_2_testFull_7_2', n_ref = 1)" \
